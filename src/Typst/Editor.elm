module Typst.Editor exposing (Model, Msg, OutMsg(..), currentSource, init, initWithSource, isBlank, recompile, requestFocus, subscriptions, update, view)
...
import Task
import Time
import Typst.Editor.Images as Images
import Typst.Editor.OutsideClick as OutsideClick
import Typst.Editor.Resize as Resize
...
init id fieldPlaceholder shortcutHint chainable preamble knownImages =
    ...
    , fieldHeight = Resize.defaultHeight
    , drag = Nothing
    , scrollTop = 0
...
update msg model =
    case msg of
        ...
        HandlePressed clientY ->
            ( { model | drag = Just (Resize.start clientY model.fieldHeight) }
            , Cmd.none
            , NoOutMsg
            )
        ...
        HandleDragged clientY ->
            case model.drag of
                Just currentDrag ->
                    ( { model | fieldHeight = Resize.drag clientY currentDrag }
                    , Cmd.none
                    , NoOutMsg
                    )
        ...
        KeyOutcome outcome ->
            case outcome of
                ...
                Keymap.Replace edit ->
                    ( model, Port.applyEdit (textareaId model) edit, NoOutMsg )
    ...
    Sub.batch
        ...
        , case model.drag of
            ...
            Nothing ->
                Sub.none
        , if model.manualEditing then
            Browser.Events.onMouseDown (OutsideClick.decoder (boxId model) OutsideClicked)
...
view model =
    div
        ...
        , div [ class "editor-preview-layer" ]
            ...
            , span [ class "editor-hint" ] [ text model.shortcutHint ]
            ]
        ]
